...
def get_package_files_list(package_name):
    ...
    out = request_pipe(cmd).decode('utf8')
...
def grant_storage_permission(package):
    read_storage_cmd = "{0} shell pm grant {1} android.permission.READ_EXTERNAL_STORAGE".format(Config.adb_path,
                                                                                                package)
    subprocess.call(read_storage_cmd, shell=True)

    write_storage_cmd = "{0} shell pm grant {1} android.permission.WRITE_EXTERNAL_STORAGE".format(Config.adb_path,
                                                                                                  package)
...
def start_instrumenting(package, release_thread=False, onstop=None, timeout=None):
    ...
    cmd = '{} shell am instrument -e coverage true {} {}/{}'.format(Config.adb_path, lock_thread, package,
                                                                    Config.INSTRUMENTING_NAME)
...
def coverage_is_locked(package_name):
    cmd = "{} shell \"test -e /mnt/sdcard/{}.lock > /dev/null 2>&1 && echo \'1\' || echo \'0\'\"".format(
        Config.adb_path, package_name)
    logging.debug('Command to check lock file:' + cmd)
    locked = subprocess.check_output(cmd, shell=True).decode('utf8').replace("\n", "").replace("\r", "")
    return locked == '1'
...
@timeit
def instrument_apk(apk_path, result_dir, dbg_start=None, dbg_end=None, installation=False,
                   granularity=Granularity.default, mem_stats=None):
    """
