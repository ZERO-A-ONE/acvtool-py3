...
class ApktoolInterface:
    def __init__(self, javaPath="java", javaOpts="-Xms512m -Xmx1024m", pathApktool=os.path.join('libs'),
                 jarApktool="apktool.jar"):
    ...
    def decode(self, apkPath, dirToDecompile, quiet=True, noSrc=False, noRes=False, debug=True, noDebugInfo=False,
               force=True, frameworkTag="", frameworkDir="", keepBrokenRes=True):
        options = "decode"
        if noSrc:
            options += " --no-src"
        if noRes:
